...
const actions = {
    ...
    submitOutlet({dispatch, commit, state}) {
        return new Promise((resolve, reject) => {
            //MENGIRIMKAN PERMINTAAN KE SERVER DAN MELAMPIRKAN DATA YANG AKAN DISIMPAN DARI STATE OUTLET
            $axios.post(`/outlets`, state.outlet)
                .then((response) => {
                    //APABILA BERHASIL KITA MELAKUKAN REQUEST LAGI UNTUK MENGAMBIL DATA TERBARU
                    dispatch('getOutlets').then(() => {
                        resolve(response.data)
                    })
                })
                .catch((error) => {
                    //APABILA TERJADI ERROR VALIDASI DIMANA LARAVEL MENGGUNAKAN CODE 422
    ...
    editOutlet({ commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.get(`outlets/${payload}/edit`)
                .then((response) => {
                    //APABILA BERHASIL, DI ASSIGN KE FORM
    ...
    updateOutlet({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            //MELAKUKAN REQUEST DENGAN MENGIRIMKAN CODE DIURL DAN MENGIRIMKAN DATA TERBARU
            //YANG TELAH DIEDIT MELALUI STATE OUTLET
    ...
    removeOutlet({ dispatch }, payload) {
        return new Promise((resolve, reject) => {
            //MENGIRIM PERMINTAAN KE SERVER UNTUK MENGHAPUS DATA DENGAN METHOD DELETE DAN ID OUTLET DI URL
